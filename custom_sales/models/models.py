...
class InheritSaleOrder(models.Model):
    ...
    confirm_rate = fields.Float(
        string='Confirm rate',
        required=False)
    currency_id_to_get_rate = fields.Many2one(
    ...
    def _action_confirm(self):
        ...
        if self.currency_id_to_get_rate.name == 'SSP':
            usd_rate = self.env['res.currency'].search([('name', '=', 'USD')]).rate
            self.confirm_rate = usd_rate
